...
//
//    This code interfaces with the potentiometer and temperature sensor. The
//    bar LEDs light up based on how much the potentiometer is turned. When PB2
//    is pressed, the temperature is read. There are messages displayed on the
//    LCD to show the potentiometer ADC value and temperature reading.
//
//*****************************************************************************
//*****************************************************************************
...
//  pressed. Additionally, when PB2 is pressed the temperature ADC value is read
//  and stored in a global variable to be processed elsewhere.
//
// INPUT PARAMETERS:
//  none
//
    ...
    switch (group_iidx_status)
    {
      ...
      case (CPUSS_INT_GROUP_IIDX_STAT_INT0):
        ...
        if ((gpio_mis & GPIO_CPU_INT_MIS_DIO15_MASK) == GPIO_CPU_INT_MIS_DIO15_SET)
        {
          ...
          // Read temperature
          temp_adc_value = ADC0_in(TEMP_SENSOR_CHANNEL);
        } /* if */
        break;
...
//  Additionally, if PB2 is pressed while the loop is running, the temperature
//  ADC value, read in the interrupt handler, is converted to Fahrenheit and
//  displayed on the LCD. The following guidelines were used to create this
//  function:
//  • Clear the LCD when the program starts.
//  • The first line of the LCD should display the ADC value: “ADC = ” followed
//    by the ADC value
//  • The second line of the LCD should display the “Temp = xxF” message. This
//    message should only update when PB2 is pressed. The temperature should not
//    change between PB2 presses.
...
//  • The design must utilize an interrupt to detect a button press.
//  • If pushbutton PB2 is pressed (detected via an interrupt), the temperature 
//    sensor is read (via channel 5).
//  • The design must utilize a while loop with a flag to monitor channel 7 of 
//    the ADC for the potentiometer.
...
//  • The program should disable interrupts and other peripherals before
//    terminating the exits main
//
// INPUT PARAMETERS:
//  none
//
...
void run_lab8_part3(void)
{
  ...
  while (!done)
  {
    ...
    asm("CPSID I"); //Set I, disable interrupts
    adc_value = ADC0_in(POT_CHANNEL);
    asm("CPSIE I"); //Clear I, enable interrupts
    ...
    if (PB2_pressed)
    {
      uint8_t temperature_c = thermistor_calc_temperature(temp_adc_value);
      uint8_t temperature_f = (temperature_c * 9/5) + 32; // convert
