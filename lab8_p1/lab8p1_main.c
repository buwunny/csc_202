...
//
//    This code reads the light sensor value and displays "Light" when the IR
//    reading is high enough and "Dark" otherwise.
//
//*****************************************************************************
//*****************************************************************************
...
//------------------------------------------------------------------------------
// DESCRIPTION:
//  This function reads the IR sensor on the board and displays the digital
//  conversion of the analog value. If the light value is above a certain value
//  the LCD displays "Light" as the status and otherwise displays "Dark". Once
//  PB1 is pressed, the loop terminates and "Program Stopped" is displayed on
//  the LCD. The following guidelines were used to create this function:
//  • All LEDs and seven-segment displays are disabled
//  • The first line of the LCD should display the status: “Status: Dark” or
//    “Status: Light”
//  • The second line of the LCD should display the ADC value: “ADC = ” followed
//    by the ADC value
//  • The design must utilize a while loop with a flag in part 1 function to
//    monitor channel 7 of the ADC continuously
...
void run_lab8_part1(void)
{
  ...
  while (!done)
  {
    ...
    adc_value = ADC0_in(PHOTO_SENSOR_CHANNEL);
    ...
    if (adc_value > LIGHT_THRESHOLD)
    {
      ...
      lcd_write_string("Dark "); // extra space to overwrite the t in "light"
    } /* else */
